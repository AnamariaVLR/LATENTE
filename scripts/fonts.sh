...
# Latin while KEEPING smcp, c2sc, onum, liga etc. Needs Python's fonttools+brotli
# (installed automatically if missing).
#
# Idempotent: exits immediately when both outputs already exist, so it is safe to
# run on every deploy (netlify.toml runs it before `astro build`). It never fails
# the build — if the tooling is unavailable the site falls back to system serifs.
#
# Usage: bash scripts/fonts.sh          (force a rebuild: FORCE=1 bash scripts/fonts.sh)
set -uo pipefail
cd "$(dirname "$0")/.."
OUT_EB="public/fonts/eb-garamond-var-latin.woff2"
OUT_CG="public/fonts/cormorant-garamond-var-latin.woff2"

if [ -z "${FORCE:-}" ] && [ -s "$OUT_EB" ] && [ -s "$OUT_CG" ]; then
...
curl -sfL "$BASE/cormorantgaramond/CormorantGaramond%5Bwght%5D.ttf" -o .fontsrc/CormorantGaramond.ttf || { echo "fonts: download failed — skipping"; exit 0; }

LATIN="U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+2000-206F,U+2074,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD"
FEATS="smcp,c2sc,onum,lnum,pnum,liga,dlig,calt,kern,mark,mkmk"

pyftsubset .fontsrc/EBGaramond.ttf        --unicodes="$LATIN" --layout-features="$FEATS" --flavor=woff2 --output-file="$OUT_EB"
pyftsubset .fontsrc/CormorantGaramond.ttf --unicodes="$LATIN" --layout-features="$FEATS" --flavor=woff2 --output-file="$OUT_CG"
ls -la "$OUT_EB" "$OUT_CG"
